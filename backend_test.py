...
class PydanticValidationTester:
    """Test suite specifically for the Pydantic validation error fix"""
    ...
    def setup_authentication(self):
        ...
        print("\n🔐 Setting up authentication for Pydantic validation tests...")
    ...
    def test_basic_health_endpoints(self):
        ...
        if success and response:
            ...
            if missing_fields:
                ...
                # Check if no Pydantic validation errors in response
                status = response.get("status")
                self.log_test("Health Status Valid", status in ["healthy", "degraded", "partial"], 
                             f"System status: {status}")
        
        # Test /health/ready (if exists)
        success, response = self.run_test(
    ...
    def test_authentication_system(self):
        ...
        if success and response:
            ...
            if missing_fields:
                ...
                self.log_test("Registration Response Structure", True, "No Pydantic validation errors")
        ...
        if self.auth_token:
            ...
            if success and response:
                ...
                if missing_fields:
                    ...
                    self.log_test("User Profile Response Structure", True, "No Pydantic validation errors")
    ...
    def test_new_api_endpoints(self):
        ...
        if success and response:
            ...
            self.log_test("Dashboard Metrics Response", True, "No Pydantic validation errors")
        
        # Test /api/backtests/run (using the correct endpoint from backend code)
        backtest_data = {
        ...
        success, response = self.run_test(
            ...
            "POST",
            "/api/backtest",  # Correct endpoint from backend code
            200,
            data=backtest_data,
            headers=auth_headers
        )
        ...
        if success and response:
            self.log_test("Backtest Run Response", True, "No Pydantic validation errors")
        ...
        if success and response:
            self.log_test("Watchlists Response", True, "No Pydantic validation errors")
    ...
    def test_error_handling(self):
        ...
        if success:
            self.log_test("Invalid Token Handling", True, "Proper 401 response")
        ...
        if success:
            self.log_test("Malformed Request Handling", True, "Proper validation error response")
    ...
    def run_all_pydantic_tests(self):
